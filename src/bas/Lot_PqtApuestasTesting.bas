...
'------------------------------------------------------------------------------*
' Procedimiento  : ComprobarSugerenciaTest
' Fecha          : sa., 07/nov/2020 11:29
' Propósito      : Casos de prueba de la clase ComprobarBoleto: Sugerencias
'------------------------------------------------------------------------------*
'
Private Sub ComprobarSugerenciaTest()
    Dim mObj As ComprobarBoletos
    Dim mSug As Sugerencia
    
  On Error GoTo ComprobarSugerenciaTest_Error
    Set mObj = New ComprobarBoletos
    Set mSug = New Sugerencia
    
    Debug.Print "==> Comprobar Sugerencia"    '
    '
    '   Sugerencia vacia
    '
    Debug.Print "    Sugerencia Vacia: " & mObj.ComprobarApuesta(mApt, False)
    PrintComprobarBoleto mObj
    
    
    '
    '   Sugerencia sin aciertos
    '
    
    With mSug
        .Combinacion.Texto = "01-02-03-04-05-06"
    End With
  ...
  On Error GoTo 0
ComprobarSugerenciaTest__CleanExit:
  Exit Sub
ComprobarSugerenciaTest_Error:
    Dim ErrNumber As Long: Dim ErrDescription As String: Dim ErrSource As String
    ErrNumber = Err.Number: ErrDescription = Err.Description: ErrSource = Err.Source
    Call HandleException(ErrNumber, ErrDescription, "Lot_PqtApuestasTesting.ComprobarSugerenciaTest", ErrSource)
    Call MsgBox(ErrDescription, vbCritical Or vbSystemModal, ThisWorkbook.Name)
    Call Trace("CERRAR")
End Sub
